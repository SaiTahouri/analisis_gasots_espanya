...
import seaborn as sns #para hacer gráficos más bonitos con menos código
...
def guardar_grafico(figura, nombre_archivo):
  ...
  figura.savefig(ruta, bbox_inches="tight")  # Guarda bien
  plt.close(figura)  # Cierra la figura correctamente
...
def gasto_por_comunidad(df):
 ...
 #  Agrupa el DataFrame por comunidad autónoma, suma los valores en la columna "Total" para cada comunidad, y ordena de mayor a menor.
  resumen = df.groupby("Comunidad autónoma de residencia")["Total"].sum().sort_values(ascending=False)
  # Eliminar los 3 primeros caracteres del nombre (código + espacio)
  ...
  guardar_grafico(fig, "gasto_por_comunidad.png")  # <-- pasar figura, no plt
...
def gasto_por_grupo(df):
  ...
  umbral = 0.03 # Umbral mínimo para mostrar individualmente ajustar el porcentaje para que sea suficientemente pequeño ♥‿♥
  ...
  # Renombrar con nombres más cortos (si existen en el diccionario)
  ...
  ax.set_title("Distribución del gasto medio por hogar en 2020 en España", 
               ...
               y=1.02) # ⬅ ajusta la distancia vertical (más bajo si es más cercano a la tarta)
...
def evolucion_gasto_periodo(df):
    ...
    guardar_grafico(fig, "evolucion_gasto_periodo.png")  # ← ahora sí guarda
